	...
	while (buf_len) {
		...
		hdr_info = FIELD_PREP(WMI_TLV_TAG, WMI_TAG_ARRAY_BYTE) |
			   FIELD_PREP(WMI_TLV_LEN, (chunk_len +
						    sizeof(struct ath12k_wmi_ftm_seg_hdr_params)));
		ftm_cmd->tlv_header = cpu_to_le32(hdr_info);
		...
		ftm_cmd->seg_hdr.segmentinfo = cpu_to_le32(seginfo);
		segnumber++;
