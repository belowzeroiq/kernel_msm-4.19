...
static int bq25790_get_vsys_adc(struct bq25790_device *bq)
{
	int ret;
	...
	ret = regmap_read(bq->regmap, BQ25790_ADC_VSYS_MSB, &vsys_adc_msb);
	if (ret)
		return ret;

	ret = regmap_read(bq->regmap, BQ25790_ADC_VSYS_LSB, &vsys_adc_lsb);
	if (ret)
		return ret;

	vsys_adc = (vsys_adc_msb << 8) | vsys_adc_lsb;

	return vsys_adc * BQ25790_ADC_VOLT_STEP_uV;
}
			     ...
			     struct bq25790_state *state)
{
	...
	state->vsys_adc = bq25790_get_vsys_adc(bq);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		val->intval = BQ25790_ICHRG_I_MAX_uA;
		break;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		val->intval = BQ25790_VREG_V_MAX_uV;
		break;
				  ...
				  struct bq25790_state *new_state)
{
	...
	return (old_state.chrg_status != new_state->chrg_status ||
		...
		old_state.vsys_adc != new_state->vsys_adc ||
...
static int bq25790_hw_init(struct bq25790_device *bq)
{
	...
	if (bq->init_data.ichg) {
		ret = bq25790_set_ichrg_curr(bq, bq->init_data.ichg);
		if (ret)
			goto err_out;
	}

	if (bq->init_data.iprechg) {
		ret = bq25790_set_prechrg_curr(bq, bq->init_data.iprechg);
		if (ret)
			goto err_out;
	}

	if (bq->init_data.vreg) {
		ret = bq25790_set_chrg_volt(bq, bq->init_data.vreg);
		if (ret)
			goto err_out;
	}

	if (bq->init_data.iterm) {
		ret = bq25790_set_term_curr(bq, bq->init_data.iterm);
		if (ret)
			goto err_out;
	}

	if (bq->init_data.vlim) {
		ret = bq25790_set_input_volt_lim(bq, bq->init_data.vlim);
		if (ret)
			goto err_out;
	}

	if (bq->init_data.ilim) {
...
static int bq25790_parse_dt(struct bq25790_device *bq)
{
	...
	ret = device_property_read_u32(bq->dev,
				       ...
				       &bq->init_data.ichg);
	if (ret)
		bq->init_data.ichg = BQ25790_ICHRG_I_DEF_uA;

	if (bq->init_data.ichg > BQ25790_ICHRG_I_MAX_uA ||
	    bq->init_data.ichg < BQ25790_ICHRG_I_MIN_uA)
		return -EINVAL;
	...
	ret = device_property_read_u32(bq->dev,
				       ...
				       &bq->init_data.vreg);
	if (ret)
		bq->init_data.vreg = BQ25790_VREG_V_DEF_uV;

	if (bq->init_data.vreg > BQ25790_VREG_V_MAX_uV ||
	    bq->init_data.vreg < BQ25790_VREG_V_MIN_uV)
		return -EINVAL;
	...
	if (ret)
		bq->init_data.iprechg = BQ25790_PRECHRG_I_DEF_uA;

	if (bq->init_data.iprechg > BQ25790_PRECHRG_I_MAX_uA ||
	    bq->init_data.iprechg < BQ25790_PRECHRG_I_MIN_uA)
		return -EINVAL;
	...
	ret = device_property_read_u32(bq->dev,
				       "termination-current-microamp",
				        &bq->init_data.iterm);
	if (ret)
		bq->init_data.iterm = BQ25790_TERMCHRG_I_DEF_uA;

	if (bq->init_data.iterm > BQ25790_TERMCHRG_I_MAX_uA ||
	    bq->init_data.iterm < BQ25790_TERMCHRG_I_MIN_uA)
		return -EINVAL;
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = bq25790_power_supply_init(bq);
	if (ret) {
